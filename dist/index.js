(function (global, factory) {
  ...
  (global = global || self, factory(global.graphqlArgs = {}));
  ...
  function astToBody(ast) {
    ...
    }, ast.variableValues, ast.fragments);
  ...
  function extractArguments(args, variables) {
    ...
    return args.reduce((acc, field) => {
      acc[field.name.value] = field.value.kind === 'ObjectValue' ? extractArguments(field.value.fields, variables) : extractArgumentValue(field.value, variables);
      return acc;
    }, {});
  }
  ...
  function extractSelectionSet(set, variables, fragments) {
    ...
    set.selections.forEach(el => {
      if (el.kind === 'FragmentSpread') {
        ...
        const selectionSet = extractSelectionSet(fragment.selectionSet, variables, fragments);
        Object.assign(body, selectionSet);
      } else if (!el.selectionSet) {
        body[el.name.value] = true;
      } else {
        body[el.name.value] = extractSelectionSet(el.selectionSet, variables, fragments);
        body[el.name.value][Symbols.ARGUMENTS] = extractArguments(el.arguments, variables);
      }
    });
    return body;
  }
  ...
  function clean(obj, maxDepth = 1, depth = 1) {
    ...
    if (typeof obj === 'object') {
      ...
      return Object.keys(obj).reduce((acc, key) => {
        ...
        acc[key] = depth < maxDepth || maxDepth < 1 ? clean(obj[key], maxDepth, depth + 1) : true;
        return acc;
      }, {});
    }
  ...
  function getAst({
    query,
    variables
  }, isAppSyncSelectionSet = false) {
    const ast = parse(query);
    const operation = ast.definitions.find(x => x.kind === 'OperationDefinition');
    const fragments = ast.definitions.filter(x => x.kind === 'FragmentDefinition').reduce((acc, fragment) => {
  ...
  const ast = getAst(example);

  function parse$1(ast) {
  ...
  function getArgs(ast, path) {
    if (typeof path === 'undefined') {
      return parse$1(ast).getArgs;
    }

    return parse$1(ast).getArgs(path);
  }
  function getFields(ast, path, options) {
    if (typeof path === 'undefined') {
      return parse$1(ast).getFields;
    }

    return parse$1(ast).getFields(path, options);
  }
